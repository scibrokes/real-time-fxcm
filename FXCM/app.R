...
getFOREX <- memoise(function(currency, type = 'Lo') {
...
ui <- shinyUI(fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      fileInput('file1', 'Upload xml File', multiple = TRUE,
                accept = c('xml', 'xml file', '.xml')), 
      sliderInput('ahead', HTML('Forecast ahead (&zeta; in min) :'), 
